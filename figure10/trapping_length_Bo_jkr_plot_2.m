...
valid = lambda_row > 0;
scatter(Bo(valid)*4/3, lambda_row(valid), size_marker, ...
        'o', ...
...
xlim([x_min, x_max]);
ylim([y_min, y_max]);
xlh =xlabel('$Bo_{\mathrm{DMT}}=4 \pi \sigma r_{\mathrm{eff}}/W$','fontsize',size_font,'Interpreter','latex');
...
text(18,0.36,'DMT','color',colors(2,:),'fontsize',size_font1,'Interpreter','latex');
text(8,0.4,'JKR','color',colors(3,:),'fontsize',size_font1,'Interpreter','latex');
% text(11,0.1,'$\mu=0.1$','color','k','fontsize',size_font1,'Interpreter','latex');
...
box on;
print(f, 'trapping_length_Bo_jkr_2.eps', '-depsc2');
print(f, 'trapping_length_Bo_jkr_2.pdf', '-dpdf', '-r300');
print(f, 'trapping_length_Bo_jkr_2.jpg', '-djpeg', '-r300');
